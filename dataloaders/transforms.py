...
class RandomMirror(object):
    ...
    def __call__(self, sample):
        img, img_slic, label = sample['image'], sample['img_slic'], sample['label']
        inst, scribble = sample['inst'], sample['scribble']
        if random.random() < 0.5:
            img = img.transpose(Image.FLIP_LEFT_RIGHT)
            img_slic = img_slic.transpose(Image.FLIP_LEFT_RIGHT)
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        img_slic = tr_F.resize(img_slic, self.size)
        if isinstance(label, dict):
...
class ToTensorNormalize(object):
    ...
    def __call__(self, sample):
        ...
        img_slic = tr_F.to_tensor(img_slic)
        img = tr_F.normalize(img, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        img_slic = tr_F.normalize(img_slic, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        if isinstance(label, dict):
