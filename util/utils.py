...
# @Czw slic
def myslic(img_cv2):
    segments_slic = slic_superpixels.slic(img_cv2, n_segments=512, compactness=30)  # [333, 500]
    # rgb_gray = rgb2gray(img_cv2)
    # regions = regionprops(segments_slic, intensity_image=rgb_gray)
    # # if len(img_cv2.shape) == 3:
    # #     regions = regionprops(segments_slic, intensity_image=rgb_gray)
    # # else:
    # #     qiepian = segments_slic[..., ]
    # #     regions = regionprops(segments_slic[..., ], intensity_image=rgb_gray)
...
def pred_query_cover(query_id, pred):
    query_path = "/home/czb/LZU_WORK_SPACE/GCC_test/data/VOCdevkit/VOC2012/JPEGImages/" + query_id + ".jpg"
