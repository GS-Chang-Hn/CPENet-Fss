...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        for epi in range(batch_size):
            ...
            slic = cv2.ximgproc.createSuperpixelSLIC(query_feture_map.detach().numpy(), region_size=20, ruler = 20.0)
            ...
            """过滤掉背景的前景特征map"""
            supp_fg_map = [[(F.interpolate(self.getFeatures(supp_fts_proj_out[way, shot, [epi]], fore_mask[way, shot, [epi]])[..., None, None],
            ...
            sup_fg_superpixels = slic(support_fg_maps.detach().numpy(), multichannel=True, n_segments=512, sigma=5)  # 448 448
            sup_fg_superpixels = torch.Tensor(sup_fg_superpixels).unsqueeze(0).unsqueeze(0).unsqueeze(0).cuda(0)  # 1 1 1 448 448 类似一个mask
            """支持图片的前景超像素"""
            supp_pixels_fts = [[self.getFeatures(supp_fts_proj_out[way, shot, [epi]],
                                             sup_fg_superpixels[way, shot, [epi]])
                            for shot in range(n_shots)] for way in range(n_ways)]  # [[ 1 512 ]] # 仅仅拿到support 前景的超像素
            """ support branch superpixels end """
            """支持图片的背景超像素 start"""
            supp_bg_map = [[(F.interpolate(self.getFeatures(supp_fts_proj_out[way, shot, [epi]], back_mask[way, shot, [epi]])[..., None, None],
            ...
            sup_bg_superpixels = slic(support_bg_maps.detach().numpy(), multichannel=True, n_segments=512, sigma=5)  # 448 448
            sup_bg_superpixels = torch.Tensor(sup_bg_superpixels).unsqueeze(0).unsqueeze(0).unsqueeze(0).cuda(0)  # 1 1 1 448 448 类似一个mask

            supp_bg_fts = [[self.handle_vit(self.vit_model(F.interpolate(
                self.getFeatures(supp_fts_proj_out[way, shot, [epi]], sup_bg_superpixels[way, shot, [epi]])[..., None, None],
