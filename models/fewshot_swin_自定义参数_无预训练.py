...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None, depth=12, act_layer=None, norm_layer=None):
        ...
        # Swin transformer
        self.swin_transformer = SwinTransformer(in_chans=3,
                                                        patch_size=2,
                                                        window_size=7,
                                                        embed_dim=64,
                                                        depths=(2, 2, 6, 2),
                                                        num_heads=(8, 16, 16, 32),
                                                        num_classes=0,
                                                        use_checkpoint=True)
        self.swin2feat = nn.Linear(28*28, 56*56)
        # self.encoder = nn.Sequential(OrderedDict([
    ...
    def forward(self, supp_imgs, supp_slic_imgs, fore_mask, back_mask, qry_imgs, qry_slic_imgs, support_images_id, query_images_id):
        """
        Args:
        ...
        qry_slic_fts_proj_out, qry_slic_fts_proj_pool, supp_slic_fts_proj_out = self.get_encode_feature(batch_size, n_queries, n_shots,
                                                                                                        n_ways, qry_slic_imgs, supp_slic_imgs)

        qry_fts_proj_out_swin, qry_fts_proj_pool_swin, supp_fts_proj_out_swin = self.get_swin_feature(batch_size, n_queries, n_shots,
                                                                                         n_ways, qry_imgs, supp_imgs)
        # @GL slic

        qry_slic_fts_proj_out_swin, qry_slic_fts_proj_pool_swin, supp_slic_fts_proj_out_swin = self.get_swin_feature(batch_size, n_queries, n_shots,
                                                                                                        n_ways, qry_slic_imgs, supp_slic_imgs)
        # torch.cat([qry_fts_proj_out, qry_slic_fts_proj_out_swin], dim=-1)
        ...
        for epi in range(batch_size):
            # 原始图片
            supp_fg_fts = [[self.getFeatures(supp_fts_proj_out_swin[way, shot, [epi]],
                                             fore_mask[way, shot, [epi]])
                            for shot in range(n_shots)] for way in range(n_ways)]  # [[ 1 512 ]]  resnet 1 1024

            # s  slic前景   supp_slic_fg_fts--》slic原始结果

            supp_slic_fg_fts = [[self.getFeatures(supp_slic_fts_proj_out_swin[way, shot, [epi]],
                                                  fore_mask[way, shot, [epi]])
                                 for shot in range(n_shots)] for way in range(n_ways)]  # [[ 1 512 ]]  resnet 1 1024
            # supp_slic_fg_fts = [[self.handle_vit(self.vit_model(F.interpolate(
            #     self.getFeatures(supp_slic_fts_proj_out[way, shot, [epi]], fore_mask[way, shot, [epi]])[..., None, None],
            ...
            #         proto_bg_feat = self.getFeatures(supp_fts_proj_out[way, shot, [epi]], back_mask[way, shot, [epi]])
            #         vit_input = F.interpolate(proto_bg_feat[..., None, None], size=back_mask.shape[-2:], mode='bilinear')  # 背景特征 1,512,448,448
            #         hand_vit_input = self.vit_model(vit_input).reshape((1, 14, 14, 512)).permute([0, 3, 2, 1])
            #         hand_vit_input = hand_vit_input + self.res_conv(vit_input)
            #         shot_temp = self.handle_vit(hand_vit_input, back_mask[way, shot, [epi]])
            #         shot_list.append(shot_temp)
            #         ress_shot_in.append(vit_input)
            #     way_list.append(shot_list)
            #     ress_in.append(torch.stack(ress_shot_in, 0))

            # bg_proto_feat_numpy = np.array(way_list)
            # supp_bg_fts = torch.from_numpy(bg_proto_feat_numpy)
            # supp_bg_fts = way_list
            # res_fts = np.array(ress_in)
            # res_fts_tensor = torch.from_numpy(res_fts.astype(torch.float))
            # res_fts_tensor = torch.stack(ress_in, 0)
            ...
            supp_slic_bg_fts = [[self.getFeatures(supp_slic_fts_proj_out_swin[way, shot, [epi]],
                                            back_mask[way, shot, [epi]])
                           for shot in range(n_shots)] for way in range(n_ways)]

            supp_bg_fts = [[self.getFeatures(supp_fts_proj_out_swin[way, shot, [epi]],
                                                  back_mask[way, shot, [epi]])
                                 for shot in range(n_shots)] for way in range(n_ways)]
            ...
            for prototype in prototypes:
                fusion_prototype_1 = att_fusion.bi_modal_attention(qry_fts_proj_pool_swin,
            ...
            for prototype_slic in slic_prototypes:
                fusion_prototype_2 = att_fusion.bi_modal_attention(qry_slic_fts_proj_pool_swin,
            ...
            for pre_i in range(len(new_prototypes_orginal_image)):
                ...
                final_prototypes.append(new_prototypes_orginal_image[pre_i].permute(1, 0) + new_prototypes_slic[pre_i].permute(1, 0))
            ...
            dist = [self.calDist(qry_fts_proj_out_swin[:, epi], prototype) for prototype in final_prototypes]
            ...
            if self.config['align'] and flag:
                align_loss_epi = self.alignLoss(qry_fts_proj_out[:, epi], pred, supp_fts_proj_out_swin[:, :, epi],
    ...
    def get_swin_feature(self, batch_size, n_queries, n_shots, n_ways, qry_imgs, supp_imgs):
        ...
        img_fts_proj_out = self.swin_transformer(imgs_concat) # out: [2, 196, 512]  out: [2, 512, 56, 56]
        c_n, c_l, c_c = img_fts_proj_out.shape
        img_fts_proj_out = self.swin2feat(img_fts_proj_out.permute(0,2,1)).view(c_n, c_c, 56, 56)
